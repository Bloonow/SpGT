...
from SpGT.common.path import DATA_PATH, FIGURE_PATH
...
for key, ax in zip(sample.keys(), iter(axs.flatten())):
    ...
    ax.axis('off')
# plt.savefig(os.path.join(FIGURE_PATH, 'darcy_insection.png'), format='png', transparent=True, dpi=360, bbox_inches='tight')

# fig, ax = plt.subplots(layout='constrained', figsize=(4, 4))
# for key in sample.keys():
#     ax.imshow(sample[key], aspect='equal')
#     ax.axis('off')
#     plt.savefig(os.path.join(FIGURE_PATH, f'{key}.png'), format='png', transparent=True, dpi=360, bbox_inches='tight')
